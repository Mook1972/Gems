...
--08172016 added code to pull service account name and assign as job owner
--09022016 added time stamp variable and print for csv
--09162016 changed from DMG_UTILITY1 to DMG_UTILITY and re tested
--09282016 changed the SQLAgent job owner from Service account to SQLADMIN account
--also added a new field for dbrole and server role stamps in csv output file
--12072016 Added logic to determine if xp_cmdshell is turned on and do not turn off when completed
--12082016 added new stamp to csv output file PROD
...
Print 'NONPROD:' + @srvname +  ','+ @dbrole2 + ','+ @ServerRole2 + ',' + @timestamp + ',' + 'SrvRole'
...
Print 'NONPROD:' + @srvname +  ','+@dbname2 + ',' + @membername + ','+   @dbrole2 + ',' + @timestamp + ',' + 'DBRole'
...
--Print @sqlstr
Select @sqlstr2 = '' sqlcmd -S''+@sqlsvr+'' -E -dMaster -Q"Exec DMG_UTILITY..Audit_DatabaseRoles_2a" >C:\AuditArtifacts\AuditArtifacts_DatabaseRole_'' + @timestamp + ''.csv''
--Print @sqlstr2
...
Print '' Check the errors in this log''',
		@database_name=N'master', 
		@output_file_name=N'C:\AuditArtifacts\AuditArtifacts_DatabaseRole_Errorlog.txt', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
Select @sqlsvrstamp = replace(@sqlsvr,''\'',''_'')  
--Print @sqlstr
Select @sqlstr2 = '' sqlcmd -S''+@sqlsvr+'' -E -dMaster -Q"Exec DMG_UTILITY..Audit_ServerRoles_2a" >C:\AuditArtifacts\AuditArtifacts_ServerRoles_'' + @timestamp + ''.csv''
--Print @sqlstr2
EXEC master..xp_cmdshell @sqlstr2
...
Print '' Check the errors in this log''', 
		@database_name=N'master', 
		@output_file_name=N'C:\AuditArtifacts\AuditArtifacts_ServerRole_Errorlog.txt', 
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
...
GO
--Select @@ServerName
Print 'PCAOB Audit objects deployed to:' + @@servername
Print 'Attemping the backup to nul'
go
BACKUP DATABASE DMG_UTILITY TO DISK = 'nul'
go
If @@ERROR != 0
Print '!!!!!!!!!!Error Backup to Nul failed !!!!!!!!!!'
