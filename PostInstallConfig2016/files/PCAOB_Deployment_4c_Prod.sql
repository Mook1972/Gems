...
as
Print @@servername + ',PCAOB Rev4c Prod'
GO
...
Print 'PROD:' + @sqlsvrstamp +  ',' + @dbrole2 + ',' + @ServerRole2 + ',' + @timestamp + ',' + 'SrvRole'
...
Print 'PROD:'  + @sqlsvrstamp +  ','+@dbname2 + ',' + @membername + ','+   @dbrole2 + ',' + @timestamp + ',' + 'DBRole'
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'AuditArtifacts', 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'The job lands Production physical artifact files on disk for auditing purposes.', 
